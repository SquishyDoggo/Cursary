...
type 	TAllVoc = array [1..MaxSize,1..3] of String; 		{ [x,y]: each x holds one vocabulary, i.e. english, kanji or hiragana and if still needed hiragana }
	...
	TMultiTrans = array [1..MultiTransSize] of String; 	{ contains multiple english translations if given }

	TVocRec = record 					{ contains number of vocabulary and themselves }
		AllVoc: TAllVoc;
		VocNum: Integer;
	end;
...
{ so that in operator can be used with type TMultiTrans }
...
begin
	...
	while pos(MultiTransDelim,trans) > 0 do 		{ while there are multiple translations }
	begin
		MultiTrans[i] := copy(trans,1,pos(MultiTransDelim,trans)-1);
		Delete(trans,1,pos(MultiTransDelim,trans));
		Inc(i);
	end;
	if ContainsMultiTrans then MultiTrans[i] := trans 	{ to get last translation as no ; exist after it and thus loop stops }
...
begin
	Assign(Dict,DictName); Reset(Dict);
	i:=1;
	repeat
		ReadLn(Dict,CurLine);
		while CurLine = '' do ReadLn(Dict,CurLine); 	{ read as long as empty lines are found }
...
var 	UserTrans, En, Ja, Hira: String;
	VocRec: TVocRec;
begin
	VocRec := GetNVoc(DictName);	
	En := VocRec.AllVoc[VocIndex][1]; Ja := VocRec.AllVoc[VocIndex][2]; Hira := VocRec.AllVoc[VocIndex][3];
	write(#10#13+En+': ');
	readln(UserTrans);
	if (UserTrans = '') and (Hira <> '') then writeln('Incorrect. Correct answer: '+Ja+' ['+Hira+']')
	else if (UserTrans = '') and (Hira = '') then writeln('Incorrect. Correct answer: '+Ja)
	else
		if (CompareText(UserTrans,Ja) <> 0) and (CompareText(UserTrans,Hira) <> 0) then 
			if Hira <> '' then writeln('Incorrect. Correct answer: '+Ja+' ['+Hira+']')
			else writeln('Incorrect. Correct answer: '+Ja)
		else if CompareText(UserTrans,Ja) = 0 then writeln('Correct.')
		else if (CompareText(UserTrans,Hira) = 0) and (Hira <> '') then
			writeln('Correct. Kanji notation exists: '+Ja+' ['+Hira+']')
...
begin
	VocRec := GetNVoc(DictName);	
	En := VocRec.AllVoc[VocIndex][1]; Ja := VocRec.AllVoc[VocIndex][2]; Hira := VocRec.AllVoc[VocIndex][3];
	MultiTrans := GetMultiTrans(En);
	if Hira <> '' then write(#10#13+Ja+' ['+Hira+']'+': ')
	else write(#10#13+Ja+': '); 
	
	readln(UserTrans);
	if (UserTrans = '') or ( not (UserTrans in MultiTrans) ) and ( CompareText(En,UserTrans) <> 0 ) then writeln('Incorrect. Correct answer: '+En)
	else if (UserTrans in MultiTrans) then writeln('Correct. All solutions: '+En)
